...
rt_inline rt_err_t rt_ipc_list_resume(rt_list_t *list)
{
    ...
    RT_DEBUG_LOG(RT_DEBUG_IPC, ("resume thread:%s\r\r\n", thread->name));
...
rt_err_t rt_sem_take(rt_sem_t sem, rt_int32_t time)
{
    ...
    RT_DEBUG_LOG(RT_DEBUG_IPC, ("thread %s take sem:%s, which value is: %d\r\r\n",
                                rt_thread_self()->name,
    ...
    else
    {
        ...
        else
        {
            ...
            RT_DEBUG_LOG(RT_DEBUG_IPC, ("sem take: suspend thread - %s\r\r\n",
                                        thread->name));
            ...
            if (time > 0)
            {
                RT_DEBUG_LOG(RT_DEBUG_IPC, ("set thread:%s to timer list\r\r\n",
                                            thread->name));
...
rt_err_t rt_sem_release(rt_sem_t sem)
{
    ...
    RT_DEBUG_LOG(RT_DEBUG_IPC, ("thread %s releases sem:%s, which value is: %d\r\r\n",
                                rt_thread_self()->name,
                                ((struct rt_object *)sem)->name,
                                sem->value));
...
rt_err_t rt_mutex_take(rt_mutex_t mutex, rt_int32_t time)
{
    ...
    RT_DEBUG_LOG(RT_DEBUG_IPC,
                 ("mutex_take: current thread %s, mutex value: %d, hold: %d\r\r\n",
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                RT_DEBUG_LOG(RT_DEBUG_IPC, ("mutex_take: suspend thread: %s\r\r\n",
                                            thread->name));
                ...
                {
                    RT_DEBUG_LOG(RT_DEBUG_IPC,
                                 ("mutex_take: start the timer of thread:%s\r\r\n",
                                  thread->name));
...
rt_err_t rt_mutex_release(rt_mutex_t mutex)
{
    ...
    RT_DEBUG_LOG(RT_DEBUG_IPC,
                 ("mutex_release:current thread %s, mutex value: %d, hold: %d\r\r\n",
                  thread->name, mutex->value, mutex->hold));
    ...
    if (mutex->hold == 0)
    {
        ...
        if (!rt_list_isempty(&mutex->parent.suspend_thread))
        {
            ...
            RT_DEBUG_LOG(RT_DEBUG_IPC, ("mutex_release: resume thread: %s\r\r\n",
                                        thread->name));
                         ...
                         rt_int32_t   timeout)
{
    ...
    while (mb->entry == mb->size)
    {
        ...
        if (timeout > 0)
        {
            ...
            RT_DEBUG_LOG(RT_DEBUG_IPC, ("mb_send_wait: start timer of thread:%s\r\r\n",
                                        thread->name));
...
rt_err_t rt_mb_recv(rt_mailbox_t mb, rt_uint32_t *value, rt_int32_t timeout)
{
    ...
    while (mb->entry == 0)
    {
        ...
        if (timeout > 0)
        {
            ...
            RT_DEBUG_LOG(RT_DEBUG_IPC, ("mb_recv: start timer of thread:%s\r\r\n",
                                        thread->name));
