...
#include <finsh.h>
int cmd_canstat(const char *can_name)
{
    ...
    if (strlen(can_name) > 0)
    {
        struct rt_can_status status;
        rt_device_t candev = rt_device_find(can_name);
        if (!candev)
        {
            rt_kprintf(" Can't find can device %s\r\n", can_name);
            return -1;
        }
        rt_kprintf(" Finded can device: %s...", can_name);
...
FINSH_FUNCTION_EXPORT(cmd_canstat, Stat Can Device Status.);
#endif
